describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    if (!button) throw new Error('button is null');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i=1; i<productItems.length; i++) {
      const shadowRootHandle = await productItems[i].getProperty('shadowRoot');
      const shadowRoot = await shadowRootHandle.asElement();
      const button = await shadowRoot.$('button');
      await button.click();
    } 
    const cartCount = await page.$('#cart-count');
    const cartCountTextHandle = await cartCount.getProperty('innerText');
    const cartCountText = await cartCountTextHandle.jsonValue();
